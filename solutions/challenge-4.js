        ...
        VariableDeclaration(path) {
            ...
            for (const declaration of path.node.declarations) {
                ...
                for (const referencePath of binding.referencePaths) {
                    referencePath.replaceWith(t.stringLiteral(declaration.init.value));
                }
            }
    ...
    traverse(ast, {
        VariableDeclaration(path) {
            ...
            for (const declaration of path.node.declarations) {
                ...
                for (const referencePath of binding.referencePaths) {
                    referencePath.replaceWith(t.identifier('window'));
                }
            }
...
function stringJoin(source) {
    ...
    function joinBinaryWithStringRecursively(node) {
        ...
        if (t.isBinaryExpression(node.left)) {
            left = joinBinaryWithStringRecursively(node.left);
        } else {
            left = node.left;
        }
        ...
        if (t.isBinaryExpression(node.right)) {
            right = joinBinaryWithStringRecursively(node.right);
        } else {
            right = node.right;
        }
